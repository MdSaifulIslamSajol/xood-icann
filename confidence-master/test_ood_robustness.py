...
class FeatureTester:
    def __init__(self, dataset: str, model: str, name="", seed= "_"):
        ...
        # print(m)
        # self.path = Path(f"results/{dataset}_{model}_seed_{seed}") #folder
        self.path = Path(f"results/aggregate_svhn_densenet/{dataset}_{model}_seed_{seed}") #folder
...
def test_ood(dataset, model, seed):
    print(f"\n\n================ Testing Features On {dataset} {model} ================", flush=True)
    ft = FeatureTester(dataset, model, "", seed)
...
if __name__ == "__main__":
    ...
    set_seed(args.seed)
    # test_ood("cifar10","densenet", seed=args.seed)
    # test_ood("cifar100","densenet", seed=args.seed)
    test_ood("svhn","densenet", seed=args.seed)
