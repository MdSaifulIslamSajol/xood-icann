...
def vector_norm(input, ord=2, dim=None, keepdim=False, dtype=None):
    return torch.norm(input, p=ord, dim=dim, keepdim=keepdim, dtype=dtype)
...
class MinMax:
    def __call__(self, x: torch.Tensor):
        ...
        amin = torch.min(x_reshaped, dim=1).values
        amax = torch.max(x_reshaped, dim=1).values
...
class Confidenciator:
    def __init__(self, model: nn.Module, transform, train_set: pd.DataFrame, features=(MinMax(),), reg=10):
        ...
        x = self.pt.fit_transform(self.scaler.fit_transform(train_set[self.feat_cols]))
        if reg < np.inf:
