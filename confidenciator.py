...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        print("flag 1.346 x",x)

        output = self.model(x)
        return output, self._features
    
    def predict(self, images):   # predict_openood  ==> COMBOOD
        """
        ...
        with torch.no_grad():
            for i, data in enumerate(images):
                # print("flag 1.343a data",data)
                print("flag 1.343a data.keys()",data.keys())

                print("flag 1.343b type(data)",type(data))


                label = data["label"]
                print("flag 1.345 type(label)",type(label))
                ...
                output.append(out)
                labels.append(label)
                data_imgs.append(data.cpu())  # save raw image tensors on CPU for consistency
        ...
        data_imgs_np = torch.cat(data_imgs).cpu().detach().numpy()
    
        return labels_np, output_np, features, data_imgs_np

    
    def predict2(self, images):   # predict_openood  ==> COMBOOD
    ...
    def predict_docu(self, images):   # predict_docu MR
        # rename this function to  predict()  when running with document datasets
...
class Confidenciator:
    ...
    # add_prediction_and_features_dl_old
    def add_prediction_and_features_dl(self, dataloader):
        ...
        # Flatten images per row: (50000, 3072)
        flat_imgs = data_imgs.reshape(data_imgs.shape[0], -1)  # (50000, 3072)
        ...
        df["Min_out"] = -np.min(pred, axis=-1)
        df = pd.concat([df, pd.DataFrame(features, index=df.index)], axis=1)
        # Add flattened image as a single column where each value is a vector
        df["data"] = list(flat_imgs)  # Each element is a numpy array of shape (3072,)
        # df["data"] = flat_imgs  # Each element is a numpy array of shape (3072,)
    ...
    # add_prediction_and_features_dl_COMBOOD # probably for COMBOOD
    def add_prediction_and_features_dl_COMBOOD(self, dataloader):
        print("\nconfidenciator.py  ==> Confidenciator.add_prediction_and_features_dl()")
        ...
        # self.extreme_value_vector = df[self.feat_cols]
        print("returning add_prediction_and_features_dl() with shape:", df.shape)
        print("flag 1.234 df.columns:", df.columns)
    ...
    def fit(self, cal: Dict[str, pd.DataFrame], c=None):
        ...
        print("flag 1.334a cal.keys() : ",cal.keys())
        ...
        if isinstance(cal, pd.DataFrame):
            ...
            # cal_img = np.stack(cal["data"].to_numpy()
            cal_img = np.vstack([img[0] for img in cal['data']])
            label = cal['label']
            print("flag 1.335 conf.fit label: ",label)
        ...
        print("flag 1.334d cal_data : ",cal_data)
        print("flag 1.334d cal_dl : ",cal_dl)
