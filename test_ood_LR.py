...
class FeatureTester: 
    ...
    def __init__(self,dataset: str, model: str, feature_model, name="", extreme= True, pen= True):
        ...
        if feature_model == "mahala":
            ...
            if not mahala_xood:
                print(" flag 2.112a OOD Data Collection For Mahala:")
            ...
            # if mahala_xood:
                ...
                print(" flag 2.112b OOD Data Collection For Mahala:")
    ...
    def fit_featuretester(self, c=None, new_cal_set=False):
        print("test_ood.py ==> FeatureTester.fit_featuretester()")
        ...
        if new_cal_set or  self.cal:
            ...
            print("flag 1.222a Creating Calibration Set", flush=True)
            # self.cal = calibration(self.data["Val"])
            self.cal = calibration(self.data["Val"])
            
        # self.cal = calibration(self.data["Val"])
        self.cal=self.data["Val"]
        # self.cal = {"data": pd.DataFrame(self.data["Val"])}
        print("flag 1.222a self.cal :\n", self.cal)

        # print("flag 1.222b (self.cal).keys() :", self.cal.keys())
        

        print("Fitting Logistic Regression ==> self.conf.fit(self.cal, c=c)", flush=True)
...
def test_ood(dataset, model, alpha):
    ...
    print(f"\n\n==> a) Calculating LR on Extreme values for {dataset} Datasets..")
    ...
    # for i in range(10):
    # ft.fit_featuretester(new_cal_set=True)
...
if __name__ == "__main__":
    ...
    sys.stdout = open("console_output_1.1.txt", "w")
